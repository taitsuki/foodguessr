...
外食のジャンルを提案する Web アプリケーションです。

[こちらのリンク](https://foodguessr.onrender.com/)からアクセスできます。

[アプリケーション概要のスライド](https://docs.google.com/presentation/d/1Ki-0K0zIT4ledNl0BPnmsOAYo0KN8dKT/edit?usp=sharing&ouid=110836525118435837580&rtpof=true&sd=true)
...
2. **システムモード**: OS の設定に合わせて自動でカラーモードが切り替わります
3. **手動モード**: 「ライト」または「ダーク」を選択して手動で切り替え

#### 機能の特徴

- **システムカラーモードの自動検知**: OS の設定（ダークモード/ライトモード）を自動で検知
- **設定の永続化**: 選択したカラーモードはローカルストレージに保存され、ページをリロードしても維持されます
- **統一された UI**: すべての UI 要素（ボタン、テキスト、背景）がカラーモードに応じて一貫して切り替わります
